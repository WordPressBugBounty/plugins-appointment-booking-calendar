...
<div class="ahb-section-container">
	<div class="ahb-section ahb-section-active" data-step="1">
        ...
        <p class="cpmvcontainer">The booking form was placed into the page <a href="<?php echo esc_attr($cpabc_postURL); ?>"><?php echo esc_html($cpabc_postURL); ?></a>.</p>
        <p class="cpmvcontainer">Now you can:</p>
        <div style="clear:both"></div>
        <button class="button button-primary cpmvcontainer" type="button" id="nextBtn" onclick="window.open('<?php echo esc_attr($cpabc_postURL); ?>');">View the Published Form</button>
        <div style="clear:both"></div>
...
<?php
  ...
  foreach ($myrows as $item)            
      echo '<option value="'.intval($item->id).'"'.($item->id==$_GET["cal"]?' selected':'').'>'.esc_html($item->uname).'</option>';
?>                
            </select>
                    </td>    
                </tr>   
