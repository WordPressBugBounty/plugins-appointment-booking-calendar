...
<div id="normal-sortables" class="meta-box-sortables">
 ...
 <div id="metabox_basic_settings" class="postbox" >
  <h3 class='hndle' style="padding:5px;"><span><?php esc_html_e('Calendar List','appointment-booking-calendar') ?> / <?php esc_html_e('Items List','appointment-booking-calendar') ?></span></h3>
  <div class="inside">
  
  
  <table cellspacing="2"> 
   <tr>
    <th align="left"><?php esc_html_e('ID','appointment-booking-calendar') ?></th><th align="left"><?php esc_html_e('Calendar Name','appointment-booking-calendar') ?></th><th align="left"><?php esc_html_e('Owner','appointment-booking-calendar') ?></th><th align="left"><?php esc_html_e('iCal Link','appointment-booking-calendar') ?></th><th align="left">&nbsp; &nbsp; <?php esc_html_e('Options','appointment-booking-calendar') ?></th><th align="left"><?php esc_html_e('Shortcode','appointment-booking-calendar') ?></th>    
   </tr> 
<?php  
...
?>
   <tr> 
    ...
    <td style="padding-left:15px;"> 
                             ...
                             <?php } ?>    
                             <input style="margin-bottom:3px;" class="button-primary button" type="button" name="calmanage_<?php echo esc_attr($item->id); ?>" value="<?php esc_html_e('Manage Settings','appointment-booking-calendar') ?>" onclick="cp_manageSettings(<?php echo esc_attr($item->id); ?>);" /> 
                             <?php if (current_user_can('manage_options')) { ?>
                             <input style="margin-bottom:3px" class="button-primary button" type="button" name="calpublish_<?php echo esc_attr($item->id); ?>" value="<?php esc_html_e('Publish','appointment-booking-calendar'); ?>" onclick="cp_publish(<?php echo esc_attr($item->id); ?>);" />   
                             <?php } ?>
                             <input style="margin-bottom:3px;" class="button" type="button" name="calbookings_<?php echo esc_attr($item->id); ?>" value="<?php esc_html_e('Bookings List','appointment-booking-calendar') ?>" onclick="cp_BookingsList(<?php echo esc_attr($item->id); ?>);" /> 
                             <input style="margin-bottom:3px;" class="button" type="button" name="calschedule_<?php echo esc_attr($item->id); ?>" value="<?php esc_html_e('Calendar Schedule','appointment-booking-calendar') ?>" onclick="cp_calendarschedule(<?php echo esc_attr($item->id); ?>);" /> 
                             <input style="margin-bottom:3px;" class="button" type="button" name="caladdbk_<?php echo esc_attr($item->id); ?>" value="<?php esc_html_e('Add Booking','appointment-booking-calendar') ?>" onclick="cp_addbk(<?php echo esc_attr($item->id); ?>);" /> 
    </td>
...
<?php if (cpabc_appointment_is_administrator()) { ?> 
 ...
 <div id="metabox_basic_settings" class="postbox" >
  <h3 class='hndle' style="padding:5px;"><span><?php esc_html_e('New Calendar','appointment-booking-calendar') ?> / <?php esc_html_e('Item','appointment-booking-calendar') ?></span></h3>
  <div class="inside"> 
   
       <?php esc_html_e('This version supports one calendar','appointment-booking-calendar') ?>. <a href="https://abc.dwbooster.com/download"><?php esc_html_e('Check the upgrade options','appointment-booking-calendar') ?></a>.
 ...
 <div id="metabox_basic_settings" class="postbox" >
  <h3 class='hndle' style="padding:5px;"><span><?php esc_html_e('Form Builder Settings & Troubleshoot Area','appointment-booking-calendar') ?></span></h3>
  <div class="inside"> 
    <p><strong><?php esc_html_e('Important!','appointment-booking-calendar') ?></strong>: <?php esc_html_e('Use this area <strong>only</strong> if you want to activate the form builder or if you are experiencing conflicts with third party plugins, with the theme scripts or with the character encoding.','appointment-booking-calendar') ?> </p>
    <form name="updatesettings">
    
      <?php esc_html_e('Form rendering','appointment-booking-calendar') ?>:<br />
       <select id="ccformrender" name="ccformrender">
        <option value="1" selected><?php esc_html_e('Use classic predefined form','appointment-booking-calendar') ?></option>        
       </select><br />
       <em>* <?php esc_html_e('The <strong>Visual Form Builder</strong> is available in the','appointment-booking-calendar') ?> <a href="https://abc.dwbooster.com/download"><?php esc_html_e('commercial versions','appointment-booking-calendar') ?></a>. <?php esc_html_e('To edit the form in this basic version you should manually edit the file','appointment-booking-calendar') ?> 'cpabc_scheduler.inc.php'.</em>
      
      <br /><br />
          
    
      <?php esc_html_e('Script load method','appointment-booking-calendar') ?>:<br />
       <select id="ccscriptload" name="ccscriptload">
        <option value="1" <?php if (get_option('CPABC_APPOINTMENTS_LOAD_SCRIPTS',(CPABC_APPOINTMENTS_DEFAULT_DEFER_SCRIPTS_LOADING?"1":"0")) == "1") echo 'selected'; ?>><?php esc_html_e('Classic (Recommended)','appointment-booking-calendar') ?></option>
        <option value="2" <?php if (get_option('CPABC_APPOINTMENTS_LOAD_SCRIPTS',(CPABC_APPOINTMENTS_DEFAULT_DEFER_SCRIPTS_LOADING?"1":"0")) != "1") echo 'selected'; ?>><?php esc_html_e('Direct','appointment-booking-calendar') ?></option>
       </select><br />
       <em>* <?php esc_html_e('Change the script load method if the form doesn\'t appear in the public website','appointment-booking-calendar') ?>.</em>
      
      <br /><br />
      <?php esc_html_e('Character encoding','appointment-booking-calendar') ?>:<br />
       <select id="cccharsets" name="cccharsets">
        <option value=""><?php esc_html_e('Keep current charset (Recommended)','appointment-booking-calendar') ?></option>
        <option value="utf8_general_ci">UTF-8 (try this first)</option>
        <option value="latin1_swedish_ci">latin1_swedish_ci</option>
       </select><br />
       <em>* <?php esc_html_e('Update the charset if you are getting problems displaying special/non-latin characters. After updated you need to edit the special characters again','appointment-booking-calendar') ?>.</em>
             
       <br /><br />
       <?php esc_html_e('iCal timezone difference vs server time','appointment-booking-calendar'); ?>:<br />
       ...
       <em>* <?php esc_html_e('Update this, if needed, to match the desired timezone. The difference is calculated referred to the server time. Current server time is','appointment-booking-calendar') ?> <?php echo esc_html(date("Y-m-d H:i")); ?></em>
       
       <br /><br />
       <?php esc_html_e('iCal timeslot size in minutes','appointment-booking-calendar') ?>:<br />
        <input type="text" size="2" name="icaltimeslotsize" id="icaltimeslotsize" value="<?php echo esc_attr(get_option('CPABC_CAL_TIME_SLOT_SIZE_SET',"30")); ?>" /> <?php esc_html_e('minutes','appointment-booking-calendar') ?>
        <br />
       <em>* <?php esc_html_e('Update this, if needed, to have a specific slot time in the exported iCal file','appointment-booking-calendar') ?>.</em>
      
       <br /><br />
       <?php esc_html_e('Exclude columns from CSV exported files','appointment-booking-calendar') ?>:<br />
        <input type="text" size="50" name="excludecolumns" id="excludecolumns" value="<?php echo esc_attr(get_option('CPABC_EXCLUDED_COLUMNS',"")); ?>" /> 
        <br />
       <em>* <?php esc_html_e('Names of the columns to be excluded, comma separated','appointment-booking-calendar') ?>.</em>
  ...
  <h3 class='hndle' style="padding:5px;"><span><?php esc_html_e('Customization Area','appointment-booking-calendar') ?></span></h3>
  <div class="inside"> 
      <p><?php esc_html_e('Use this area to add custom CSS styles or custom scripts. These styles and scripts will be keep safe even after updating the plugin','appointment-booking-calendar') ?>.</p>
      <input type="button" onclick="cp_editArea('css');" name="gobtn3" value="Add Custom Styles" />
  </div>    
 </div> 
 
 <div id="metabox_basic_settings" class="postbox">
	<h3 class="hndle" style="padding:5px;"><span>Add-ons Area</span> (<a style="font-size:100%" href="https://abc.dwbooster.com/download"><strong>Platinum version only</strong> - <?php esc_html_e('Check the upgrade option','appointment-booking-calendar') ?>s</a>)</h3>
...
[<a href="https://wordpress.org/support/plugin/appointment-booking-calendar#new-post" target="_blank"><?php esc_html_e('Support','appointment-booking-calendar') ?></a>] | [<a href="https://abc.dwbooster.com/support?ref=callist" target="_blank"><?php esc_html_e('Documentation','appointment-booking-calendar') ?></a>]
</form>
</div>